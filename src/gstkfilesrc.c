 ...
 * Theory of Operation
 *
 * Update: see GstKFileSrc:use-mmap property documentation below
...
GST_DEBUG_CATEGORY_STATIC (gst_kfilesrc_debug);
#define GST_CAT_DEFAULT gst_kfilesrc_debug
...
static void gst_kfilesrc_finalize (GObject * object);

static void gst_kfilesrc_set_property (GObject * object, guint prop_id,
    const GValue * value, GParamSpec * pspec);
static void gst_kfilesrc_get_property (GObject * object, guint prop_id,
    GValue * value, GParamSpec * pspec);

static gboolean gst_kfilesrc_start (GstBaseSrc * basesrc);
static gboolean gst_kfilesrc_stop (GstBaseSrc * basesrc);

static gboolean gst_kfilesrc_is_seekable (GstBaseSrc * src);
static gboolean gst_kfilesrc_get_size (GstBaseSrc * src, guint64 * size);
static GstFlowReturn gst_kfilesrc_create (GstBaseSrc * src, guint64 offset,
    guint length, GstBuffer ** buffer);
static gboolean gst_kfilesrc_query (GstBaseSrc * src, GstQuery * query);

static void gst_kfilesrc_uri_handler_init (gpointer g_iface,
    gpointer iface_data);

static void
_do_init (GType kfilesrc_type)
{
  static const GInterfaceInfo urihandler_info = {
    gst_kfilesrc_uri_handler_init,
    NULL,
    NULL
  };

  g_type_add_interface_static (kfilesrc_type, GST_TYPE_URI_HANDLER,
      &urihandler_info);
  GST_DEBUG_CATEGORY_INIT (gst_kfilesrc_debug, "kfilesrc", 0, "kfilesrc element");
}

GST_BOILERPLATE_FULL (GstKFileSrc, gst_kfilesrc, GstBaseSrc, GST_TYPE_BASE_SRC, _do_init);

static gboolean
kfilesrc_plugin_init (GstPlugin * plugin)
{
  if (!gst_element_register (plugin, "kfilesrc", GST_RANK_PRIMARY, gst_kfilesrc_get_type()))
      return FALSE;
...
static void
gst_kfilesrc_base_init (gpointer g_class)
{
  ...
  gst_element_class_set_details_simple (gstelement_class,
      ...
      "Tff Object <tffobject@gmail.com>");
  gst_element_class_add_pad_template (gstelement_class,
      gst_static_pad_template_get (&srctemplate));
}

static void
gst_kfilesrc_class_init (GstKFileSrcClass * klass)
{
  ...
  gobject_class->set_property = gst_kfilesrc_set_property;
  gobject_class->get_property = gst_kfilesrc_get_property;
  ...
  gobject_class->finalize = gst_kfilesrc_finalize;

  gstbasesrc_class->start = GST_DEBUG_FUNCPTR (gst_kfilesrc_start);
  gstbasesrc_class->stop = GST_DEBUG_FUNCPTR (gst_kfilesrc_stop);
  gstbasesrc_class->is_seekable = GST_DEBUG_FUNCPTR (gst_kfilesrc_is_seekable);
  gstbasesrc_class->get_size = GST_DEBUG_FUNCPTR (gst_kfilesrc_get_size);
  gstbasesrc_class->create = GST_DEBUG_FUNCPTR (gst_kfilesrc_create);
  gstbasesrc_class->query = GST_DEBUG_FUNCPTR (gst_kfilesrc_query);
...
static void
gst_kfilesrc_init (GstKFileSrc * src, GstKFileSrcClass * g_class)
{
#ifdef HAVE_MMAP
...
static void
gst_kfilesrc_finalize (GObject * object)
{
  GstKFileSrc *src;

  src = GST_KFILE_SRC (object);
...
static void
gst_kfilesrc_set_property (GObject * object, guint prop_id,
    const GValue * value, GParamSpec * pspec)
{
  GstKFileSrc *src;

  g_return_if_fail (GST_IS_KFILE_SRC (object));

  src = GST_KFILE_SRC (object);

  switch (prop_id) {
    case ARG_LOCATION:
      gst_kfilesrc_set_location (src, g_value_get_string (value));
      break;
    case ARG_MMAPSIZE:
...
static void
gst_kfilesrc_get_property (GObject * object, guint prop_id, GValue * value,
    GParamSpec * pspec)
{
  GstKFileSrc *src;
...
static GstBuffer *
gst_kfilesrc_map_region (GstKFileSrc * src, off_t offset, gsize size,
    gboolean testonly)
{
  GstBuffer *buf;
  void *mmapregion;
...
static GstBuffer *
gst_kfilesrc_map_small_region (GstKFileSrc * src, off_t offset, gsize size)
{
  GstBuffer *ret;
  off_t mod;
  guint pagesize;
  ...
  if (mod != 0) {
    ...
    map = gst_kfilesrc_map_region (src, mapbase, mapsize, FALSE);
    if (map == NULL)
      return NULL;
    ...
    ret = gst_kfilesrc_map_region (src, offset, size, FALSE);
  }
...
gst_kfilesrc_create_mmap (GstKFileSrc * src, guint64 offset, guint length,
    GstBuffer ** buffer)
{
  GstBuffer *buf = NULL;
  ...
  if (offset >= mapstart) {
    ...
    if (readend <= mapend) {
      ...
      GST_LOG_OBJECT (src, "read buf %" G_GUINT64_FORMAT "+%u starts in "
          ...
          offset, (guint) readsize, (guint) mapstart, (guint) mapsize);
      buf = gst_kfilesrc_map_small_region (src, offset, readsize);
      if (buf == NULL)
        goto could_not_mmap;
    }
  ...
  if (buf == NULL) {
    ...
    if ((offset / src->mapsize) != (readend / src->mapsize)) {
      ...
      /* create a new one */
      src->mapbuf = gst_kfilesrc_map_region (src, nextmap, mapsize, FALSE);
      if (src->mapbuf == NULL)
        goto could_not_mmap;
...
static GstFlowReturn
gst_kfilesrc_create_read (GstKFileSrc * src, guint64 offset, guint length,
    GstBuffer ** buffer)
{
  int ret;
  GstBuffer *buf;
...
static GstFlowReturn
gst_kfilesrc_create (GstBaseSrc * basesrc, guint64 offset, guint length,
    GstBuffer ** buffer)
{
  GstKFileSrc *src;
  GstFlowReturn ret;

  src = GST_KFILE_SRC_CAST (basesrc);

#ifdef HAVE_MMAP
  if (src->using_mmap) {
    ret = gst_kfilesrc_create_mmap (src, offset, length, buffer);
  } else {
    ret = gst_kfilesrc_create_read (src, offset, length, buffer);
  }
#else
  ret = gst_kfilesrc_create_read (src, offset, length, buffer);
#endif
...
static gboolean
gst_kfilesrc_query (GstBaseSrc * basesrc, GstQuery * query)
{
  gboolean ret = FALSE;
  GstKFileSrc *src = GST_KFILE_SRC (basesrc);
...
static gboolean
gst_kfilesrc_is_seekable (GstBaseSrc * basesrc)
{
  GstKFileSrc *src = GST_KFILE_SRC (basesrc);
...
static gboolean
gst_kfilesrc_get_size (GstBaseSrc * basesrc, guint64 * size)
{
  struct stat stat_results;
  GstKFileSrc *src;

  src = GST_KFILE_SRC (basesrc);
...
static gboolean
gst_kfilesrc_start (GstBaseSrc * basesrc)
{
  GstKFileSrc *src = GST_KFILE_SRC (basesrc);
  struct stat stat_results;
  ...
  if (src->use_mmap) {
    ...
    src->mapbuf = gst_kfilesrc_map_region (src, 0, src->mapsize, TRUE);
    if (src->mapbuf != NULL) {
...
static gboolean
gst_kfilesrc_stop (GstBaseSrc * basesrc)
{
  GstKFileSrc *src = GST_KFILE_SRC (basesrc);
...
static GstURIType
gst_kfilesrc_uri_get_type (void)
{
  return GST_URI_SRC;
}

static gchar **
gst_kfilesrc_uri_get_protocols (void)
{
...
static const gchar *
gst_kfilesrc_uri_get_uri (GstURIHandler * handler)
{
  GstKFileSrc *src = GST_KFILE_SRC (handler);

  return src->uri;
}

static gboolean
gst_kfilesrc_uri_set_uri (GstURIHandler * handler, const gchar * uri)
{
  ...
  if (strcmp (uri, "file://") == 0) {
    ...
    gst_kfilesrc_set_location (src, NULL);
    return TRUE;
  }
  ...
  if (!location || error) {
    if (error) {
      GST_WARNING_OBJECT (src, "Invalid URI '%s' for kfilesrc: %s", uri,
          error->message);
      g_error_free (error);
    } else {
      GST_WARNING_OBJECT (src, "Invalid URI '%s' for kfilesrc", uri);
    }
    goto beach;
  }
  ...
  if ((hostname) && (strcmp (hostname, "localhost"))) {
    ...
    GST_WARNING_OBJECT (src, "Invalid hostname '%s' for kfilesrc", hostname);
    goto beach;
  }
#ifdef G_OS_WIN32
  ...
  ret = gst_kfilesrc_set_location (src, location);
...
static void
gst_kfilesrc_uri_handler_init (gpointer g_iface, gpointer iface_data)
{
  GstURIHandlerInterface *iface = (GstURIHandlerInterface *) g_iface;

  iface->get_type = gst_kfilesrc_uri_get_type;
  iface->get_protocols = gst_kfilesrc_uri_get_protocols;
  iface->get_uri = gst_kfilesrc_uri_get_uri;
  iface->set_uri = gst_kfilesrc_uri_set_uri;
}

static gboolean
plugin_init (GstPlugin * plugin)
{
  if (!gst_kfilesrc_plugin_init (plugin))
    return FALSE;
...
GST_PLUGIN_DEFINE (GST_VERSION_MAJOR,
    GST_VERSION_MINOR,
    "kfilesrc",
    "Reloadable file reader",
    kfilesrc_plugin_init, VERSION, "LGPL", "KFileSrc", "http://giss.tv")
